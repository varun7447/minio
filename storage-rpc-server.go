...
func (s *storageServer) ReadFileHandler(arg *ReadFileArgs, reply *ReadFileReply) error {
	endpoint := "http://localhost:9000/minio/storage" // TODO fix this.
...
func registerStorageServer(mux *router.Router, diskPath string) {
	...
	storageRouter.Methods("POST").Path("/storage/upload/{volume}/{path:.+}").HandlerFunc(stServer.StreamUploadHandler)
	storageRouter.Methods("GET").Path("/storage/download/{volume}/{path:.+}").Queries("offset", "").HandlerFunc(stServer.StreamDownloadHandler)
}
