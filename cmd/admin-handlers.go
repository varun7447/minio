...
func (a adminAPIHandlers) ServiceStopNRestartHandler(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		logger.LogIf(context.Background(), fmt.Errorf("Invalid service action received"))
		return
	}
...
func (a adminAPIHandlers) ServerInfoHandler(w http.ResponseWriter, r *http.Request) {
	...
	for i, p := range globalAdminPeers {
		...
		go func(idx int, peer adminPeer) {
			...
			if err != nil {
				ctx := logger.ContextSet(context.Background(), (&logger.ReqInfo{}).AppendTags("peerAddress", peer.addr))
...
func (a adminAPIHandlers) UpdateCredentialsHandler(w http.ResponseWriter,
	...
	for peer, err := range updateErrs {
		ctx := logger.ContextSet(context.Background(), (&logger.ReqInfo{}).AppendTags("peerAddress", peer))
		logger.LogIf(ctx, err)
	}
