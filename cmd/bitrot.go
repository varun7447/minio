...
func bitrotReaderEndOffset(offset int64, length int64, fileLength int64, erasureBlockSize int64, dataBlocks int) int64 {
	shardSize := ceilFrac(erasureBlockSize, int64(dataBlocks))
	shardFileSize := getErasureShardFileSize(erasureBlockSize, fileLength, dataBlocks)
	endShard := offset + int64(length)/erasureBlockSize
...
// Read returns requested data.
func (b *bitrotReader) Read(offset int64, length int64) ([]byte, error) {
	if b.buf == nil {
