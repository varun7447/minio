...
func TestServerConfigWithEnvs(t *testing.T) {
	...
	os.Setenv("MINIO_VIRTUAL_HOST_DOMAIN", "domain.com")
	defer os.Unsetenv("MINIO_VIRTUAL_HOST_DOMAIN")
	...
	if serverConfig.VirtualHostDomain != "domain.com" {
		t.Errorf("Expecting VirtualHostDomain to be `domain.com` found " + serverConfig.VirtualHostDomain)
	}
}
