...
var (
	...
	regexpDistributed = regexp.MustCompile(`(^https?://.*)({[0-9]*\.\.\.[0-9]*})(.*)({[0-9]*\.\.\.[0-9]*})`)

	// Regex to extract ellipses syntax for standalone setup.
	regexpStandalone = regexp.MustCompile(`(.*)({[0-9]*\.\.\.[0-9]*})`)
...
func parseEndpointSet(arg string) (s endpointSet, err error) {
	...
	parts := regexpDistributed.FindStringSubmatch(arg)
	if len(parts) == 0 {
		parts = regexpStandalone.FindStringSubmatch(arg)
		if len(parts) == 0 {
			return s, fmt.Errorf("Invalid input arg %s", arg)
		}
	}
...
func hasArgsWithEllipses(args ...string) (ok bool) {
	...
	for _, arg := range args {
		ok = ok && func() bool {
			var parts []string
			if parts = regexpDistributed.FindStringSubmatch(arg); len(parts) == 0 {
				return len(regexpStandalone.FindStringSubmatch(arg)) != 0
			}
			return len(parts) != 0
		}()
	}
