	...
	if ips, err = net.LookupIP(host); err != nil {
		...
		for range newRetryTimerSimple(doneCh) {
			...
			if timeElapsed > time.Second {
				...
				// resolveable.
				ctx := logger.ContextSet(context.Background(), (&logger.ReqInfo{}).AppendTags("host", host).AppendTags("elapsedTime", humanize.RelTime(startTime, startTime.Add(timeElapsed), "elapsed", "")))
