...
func newNetworkStorage(address string) (StorageAPI, error) {
	rpcClient, err := dialHTTPPath(address, reservedBucket+"/storage")
	if err != nil {
		return nil, err
	}
	httpClient := &http.Client{
...
// dialHTTPPath connects to an HTTP RPC server at the specified
// network address and path. Additionally there is a 30secs Timeout is
// the maximum amount of time a dial will wait for a connect to complete.
func dialHTTPPath(address, path string) (*rpc.Client, error) {
	var err error
